...
function Run({ sel, onPlayAgain }: { sel: LeagueSelection; onPlayAgain: () => void }) {
  ...
  function endRun(finalStreak: number) {
    if (!recorded.current) {
      ...
      trackGameEnd('rapid', sel, { streak: finalStreak, isRecord: finalStreak > initialBest && finalStreak > 0 });
    }
    setPhase('over');
  }
  ...
  if (phase === 'intro') {
    return <Intro sel={sel} best={initialBest} onStart={() => { haptic('light'); trackGameStart('rapid', sel); setPhase('play'); }} />;
  }
  if (phase === 'over') {
...
function TeamChoice({
  ...
  const style = teamStyle(league, team);
  ...
  return (
    ...
    >
      <TeamLogo league={league} team={team} size={52} />
      <div className="min-w-0 flex-1">
        <div className="truncate font-display text-3xl tracking-wide">{shortName(league, team)}</div>
