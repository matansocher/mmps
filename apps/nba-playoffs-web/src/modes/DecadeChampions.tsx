...
import { seasonByYear, FIRST_SEASON, LAST_SEASON } from '../lib/playoffs';
import { recordDecade, loadProfile } from '../lib/storage';
...
// Latest valid start year so the 10-season window still fits our data.
const LATEST_START = LAST_SEASON - WINDOW + 1;

function randomStartYear(): number {
  return FIRST_SEASON + Math.floor(Math.random() * (LATEST_START - FIRST_SEASON + 1));
}
...
export function DecadeChampions() {
  const [seed, setSeed] = useState(0);
  return <Round key={seed} onPlayAgain={() => setSeed((s) => s + 1)} />;
}

function Round({ onPlayAgain }: { onPlayAgain: () => void }) {
  const startYear = useMemo(() => randomStartYear(), []);
  const endYear = startYear + WINDOW - 1;
  const years = useMemo(() => Array.from({ length: WINDOW }, (_, i) => startYear + i), [startYear]);
  const answer = useMemo(() => years.map((y) => seasonByYear(y)!.champion), [years]);
  ...
  if (phase === 'intro') {
    return <Intro startYear={startYear} endYear={endYear} best={loadProfile().bestDecadeScore} onStart={() => { haptic('light'); setPhase('play'); }} />;
  }
  if (phase === 'result') {
    return <Result startYear={startYear} endYear={endYear} years={years} answer={answer} order={order} score={finalScore} timedOut={timedOut} onPlayAgain={onPlayAgain} />;
  }
...
function ChampRow({ chip }: { chip: Chip }) {
  ...
  const style = teamStyle(chip.team);
  return (
    <Reorder.Item
      value={chip}
      dragListener={false}
    ...
    >
      ...
      <TeamLogo team={chip.team} size={28} />
      <span className="flex-1 truncate text-sm font-semibold">{shortName(chip.team)}</span>
    </Reorder.Item>
  );
}

function Intro({ startYear, endYear, best, onStart }: { startYear: number; endYear: number; best: number; onStart: () => void }) {
  return (
    <div className="flex min-h-full flex-col">
      ...
      <div className="mx-auto flex w-full max-w-md flex-1 flex-col items-center justify-center px-6 text-center safe-b">
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', stiffness: 260, damping: 20 }}>
          <div className="rounded-full bg-flame/20 px-3 py-1 text-xs font-bold uppercase tracking-wider text-flame">Title Ladder</div>
          ...
          {years.map((y, i) => {
            ...
            return (
              ...
              >
                ...
                <TeamLogo team={answer[i]} size={30} />
                <div className="min-w-0 flex-1">
                  <div className="truncate text-sm font-semibold">{shortName(answer[i])}</div>
                  {!correct && <div className="truncate text-[11px] text-miss">you placed {picked ? shortName(picked) : '—'}</div>}
                </div>
