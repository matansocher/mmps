...
import type { FlatSeries } from '../types';
import { dailySeasonYear } from '../lib/daily';
import { seasonByYear, flattenSeason, ROUND_ORDER, roundWeight } from '../lib/playoffs';
...
function buildFeeders(items: Item[]): Record<number, number[]> {
  const map: Record<number, number[]> = {};
  for (const it of items) {
    const s = it.series;
    const ri = (ROUND_ORDER as readonly string[]).indexOf(s.round);
    if (ri <= 0) {
      map[it.index] = [];
      continue;
    }
...
export function DailyBracket() {
  ...
  const feeders = useMemo(() => buildFeeders(withIdx), [withIdx]);
  const groups = useMemo<RoundGroup[]>(
    () => ROUND_ORDER.map((round) => ({ round, items: withIdx.filter((x) => x.series.round === round) })).filter((g) => g.items.length > 0),
    [withIdx],
  );

  const trees = useMemo(() => {
    const byIdx = new Map(withIdx.map((x) => [x.index, x] as const));
    const find = (conf: 'Eastern' | 'Western') => withIdx.find((x) => x.series.round === 'Conference Finals' && x.series.conference === conf);
    const eastCF = find('Eastern');
    const westCF = find('Western');
    const finals = withIdx.find((x) => x.series.round === 'NBA Finals');
    return {
      east: eastCF ? buildTree(eastCF, byIdx, feeders) : null,
      west: westCF ? buildTree(westCF, byIdx, feeders) : null,
      finals: finals ?? null,
    };
  }, [withIdx, feeders]);

  const existing = todaysRecord(loadProfile());
  const [phase, setPhase] = useState<Phase>(existing ? 'result' : 'intro');
  const [picks, setPicks] = useState<Record<number, string>>({});
  const [flash, setFlash] = useState<Flash>(null);
  const [streak, setStreak] = useState(() => loadProfile().dailyStreak);

  const scored = useMemo(() => scorePicks(flat, picks), [flat, picks]);
  ...
  function submit() {
    ...
    const p = recordDaily({ year, score: scored.score, maxScore: scored.maxScore, correctCount: scored.correctCount, total: flat.length, flags });
    setStreak(p.dailyStreak);
    haptic('heavy');
    setPhase('result');
  }

  if (phase === 'intro') {
    return <Intro year={year} champion={season.champion} format={season.format} onStart={() => { haptic('light'); setPhase('play'); }} />;
  }
  if (phase === 'result') {
    return <Result year={year} champion={season.champion} flat={flat} groups={groups} picks={picks} streak={streak} storedFlags={existing?.flags} />;
  }

  const branchProps = { picks, stateOf, onPick, flash };
  ...
  return (
    <div className="flex min-h-full flex-col">
      ...
      <div className="pointer-events-none flex items-center justify-between px-4 pt-2 text-[11px] font-bold uppercase tracking-widest">
        <span className="text-hoop">◄ East</span>
        <span className="text-ink-muted">Finals</span>
        <span className="text-flame">West ►</span>
      </div>
      ...
      <div className="mx-auto w-full max-w-md px-4 pb-6 safe-b">
        {allPicked ? (
          ...
          <div className="rounded-2xl bg-court-card py-3 text-center text-sm font-semibold text-ink-muted">
            Tap the winner of each series — the champ advances. {total - pickedCount} left
          </div>
        )}
      </div>
    </div>
      ...
      <div className="flex w-max min-w-full items-center px-4 py-6">
        ...
        {finals && (
          <div className="flex items-center">
            <Connector side="east" />
            <MatchCard item={finals} side="east" state={finalsState} picked={bp.picks[finals.index]} flash={bp.flash} onPick={bp.onPick} tier="finals" />
            <Connector side="west" />
          </div>
        )}
...
function Branch({ node, side, ...bp }: { node: TNode; side: Side } & BranchProps) {
  ...
  const tier = node.item.series.round === 'Conference Finals' ? 'conf' : 'normal';
  ...
  return (
    <div className={`flex items-center ${side === 'west' ? 'flex-row-reverse' : ''}`}>
      ...
      <MatchCard item={node.item} side={side} state={bp.stateOf(node.item.index)} picked={bp.picks[node.item.index]} flash={bp.flash} onPick={bp.onPick} tier={tier} />
    </div>
  );
}
    ...
    >
      {(tier === 'finals' || tier === 'conf') && (
        <div className={`px-2 py-0.5 text-center text-[8px] font-bold uppercase tracking-wider ${tier === 'finals' ? 'bg-hoop/25 text-hoop' : 'bg-court-card text-ink-muted'}`}>
          {tier === 'finals' ? '🏆 Finals' : 'Conf Finals'}
        </div>
      )}

      {locked ? (
        <div>
          <SlotRow kind="placeholder" />
        ...
        <motion.div key="teams" initial={{ opacity: 0, x: side === 'west' ? -10 : 10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25 }}>
          <SlotRow kind={rowKind(s.higherSeed.team)} team={s.higherSeed.team} seed={s.higherSeed.seed} onClick={() => onPick(item.index, s.higherSeed.team, winner)} />
          <div className="mx-2 h-px bg-line-subtle" />
          <SlotRow kind={rowKind(s.lowerSeed.team)} team={s.lowerSeed.team} seed={s.lowerSeed.seed} onClick={() => onPick(item.index, s.lowerSeed.team, winner)} />
        </motion.div>
      )}
...
function SlotRow({ kind, team, seed, onClick }: { kind: SlotKind; team?: string; seed?: string; onClick?: () => void }) {
  ...
  return (
    ...
    >
      ...
      <span className={`flex-1 truncate text-[11px] font-semibold leading-none ${nameColor}`}>{shortName(team)}</span>
...
function Intro({ year, champion, format, onStart }: { year: number; champion: string; format: string; onStart: () => void }) {
  const cs = teamStyle(champion);
  return (
    <div className="flex min-h-full flex-col">
      ...
      <div className="mx-auto flex w-full max-w-md flex-1 flex-col items-center justify-center px-6 text-center safe-b">
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', stiffness: 260, damping: 20 }}>
          <div className="rounded-full bg-hoop/20 px-3 py-1 text-xs font-bold uppercase tracking-wider text-hoop">Today’s Bracket</div>
          <div className="mt-4 font-display text-8xl leading-none tracking-wide" style={{ color: cs.primary === '#000000' ? '#FFFFFF' : cs.secondary }}>
            {year}
          </div>
          <div className="mt-2 font-display text-3xl tracking-wide">NBA Playoffs</div>
        </motion.div>
        <p className="mt-4 max-w-xs text-ink-secondary">
          Fill the real bracket. Tap who won each series — get it right and your pick advances; miss it and the real winner marches on. Scroll East ↔ West and crown the champ.
        </p>
...
function Result({
  ...
  const score = hasPicks ? scored.score : flat.reduce((n, s, i) => n + (storedFlags?.[i] ? roundWeight(s.round) : 0), 0);
  ...
  async function onShare() {
    ...
    const text = buildShareText(year, results, score, maxScore, streak);
              ...
              <div className="space-y-2">
                {g.items.map(({ series, index }, i) => (
                  <motion.div key={index} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + i * 0.03 }}>
                    <RevealRow series={series} correct={flagFor(index)} picked={picks[index]} />
                  </motion.div>
                ))}
              </div>
            </div>
...
function RevealRow({ series, correct, picked }: { series: FlatSeries; correct: boolean; picked?: string }) {
  ...
  return (
    <div className={`rounded-2xl bg-court-card p-2 ring-1 ${correct ? 'ring-win/40' : 'ring-miss/40'}`}>
      <TeamCard team={winner.team} seed={winner.seed} state="correct" />
      <div className="py-0.5 text-center text-[10px] font-bold uppercase tracking-widest text-ink-muted">
        {series.result} {picked && !correct ? `· you picked ${shortName(picked)}` : ''}
      </div>
      <TeamCard team={loser.team} seed={loser.seed} state={picked === loser.team ? 'wrong' : 'dim'} />
    </div>
  );
}
