...
import type { LineupPlayer, LineupSideData } from '../lib/types';

type Props = {
  home: { name: string; lineup: LineupSideData };
  away: { name: string; lineup: LineupSideData };
};
  ...
  return (
    <div className="grid grid-cols-2 divide-x divide-border-subtle border-b border-border-subtle">
      <div className="px-3 py-2 text-right">
        <div className="text-text-primary text-sm font-semibold truncate">{home.name}</div>
        {home.lineup.formation && <div className="text-text-secondary text-xs">{home.lineup.formation}</div>}
      </div>
      <div className="px-3 py-2 text-left">
        <div className="text-text-primary text-sm font-semibold truncate">{away.name}</div>
...
function SideList({ players, align }: { players: readonly LineupPlayer[]; align: 'start' | 'end' }) {
  if (players.length === 0) return <div className="px-3 py-3 text-text-muted text-xs">—</div>;
  return (
