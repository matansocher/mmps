...
export function MonthPicker({ selected, onSelect }: Props) {
  const isCurrent = selected === currentYm();
  return (
    <div className="flex items-center justify-between gap-2">
      <NavButton dir="prev" onClick={() => onSelect(shiftMonth(selected, -1))} />
      <div className="flex-1 px-4 py-2 rounded-full bg-bg-card border border-border-subtle text-text-primary font-medium text-sm flex items-center justify-center gap-2">
        ...
        <span>{formatMonthLabel(selected)}</span>
      </div>
      <NavButton dir="next" onClick={() => onSelect(shiftMonth(selected, 1))} disabled={isCurrent} />
    </div>
  );
}
