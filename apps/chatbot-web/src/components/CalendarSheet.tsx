...
function cellClass(cell: DayCell): string {
  const base = 'h-10 rounded-lg text-sm tabular transition-colors flex items-center justify-center';
  if (cell.isSelected) {
    return `${base} bg-accent-primary text-white font-bold`;
  }
  if (cell.isToday) {
    return `${base} text-accent-primary ring-1 ring-accent-primary/60 hover:bg-bg-elevated`;
  }
...
function Chevron({ dir }: { readonly dir: 'prev' | 'next' }) {
