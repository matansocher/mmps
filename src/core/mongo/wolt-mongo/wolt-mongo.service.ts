...
export class WoltMongoService implements OnModuleInit {
  ...
  subscriptionCollection: Collection<SubscriptionModel>;
  ...
  async connectToMongo(): Promise<void> {
    try {
      ...
      this.subscriptionCollection = DB.collection<SubscriptionModel>(mongoConfig.WOLT.COLLECTIONS.SUBSCRIPTIONS);
      this.userCollection = DB.collection<UserModel>(mongoConfig.WOLT.COLLECTIONS.USER);
      this.analyticLogCollection = DB.collection<AnalyticLogModel>(mongoConfig.WOLT.COLLECTIONS.ANALYTIC_LOGS);
    } catch (err) {
