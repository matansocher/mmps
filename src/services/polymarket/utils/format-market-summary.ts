...
export function formatMarketSummary(market: MarketSummary): string {
  ...
  const changeStr =
    market.oneDayPriceChange !== null ? `${market.oneDayPriceChange >= 0 ? '+' : ''}${(market.oneDayPriceChange * 100).toFixed(1)}%` : 'N/A'
...
export function formatMarketForList(market: MarketSummary): string {
  const yesPct = (market.yesPrice * 100).toFixed(1)
  const changeStr =
    market.oneDayPriceChange !== null ? `(${market.oneDayPriceChange >= 0 ? '+' : ''}${(market.oneDayPriceChange * 100).toFixed(1)}%)` : ''
