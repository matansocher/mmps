...
export class PinBuddyService {
  ...
  async handleReplyToMessage({
    ...
    try {
      await this.telegramGeneralService.sendChatAction(
        this.bot,
        chatId,
        BOT_BROADCAST_ACTIONS.TYPING,
      );

      const existingPin = await this.mongoPinService.getPin(
        chatId,
        replyToMessageId,
      );
      if (existingPin) {
        const { message_id: temporaryMessageAfterCheckId } =
          await this.telegramGeneralService.sendMessage(
            this.bot,
            chatId,
            'Sorry, but this message is already pinned',
        ...
        await this.telegramGeneralService.deleteMessage(
          this.bot,
          chatId,
          temporaryMessageAfterCheckId,
        );
        return;
      }

      const { message_id: temporaryMessageBeforeSaveId } =
        await this.telegramGeneralService.sendMessage(
          this.bot,
          chatId,
          'OK, I will save this message for you, just a sec...',
          await this.pinBuddyUtilsService.getKeyboardOptions(chatId),
        );
      ...
      const { message_id: temporaryMessageAfterSaveId } =
        await this.telegramGeneralService.sendMessage(
          this.bot,
          chatId,
          `OK, I have saved this message for you - ${text}`,
          await this.pinBuddyUtilsService.getKeyboardOptions(chatId),
        );
      ...
      await Promise.all([
        ...
        this.telegramGeneralService.deleteMessage(
          this.bot,
          chatId,
          temporaryMessageBeforeSaveId,
        ),
        this.telegramGeneralService.deleteMessage(
          this.bot,
          chatId,
          temporaryMessageAfterSaveId,
        ),
      ]);
      ...
      this.logger.error(
        this.handleReplyToMessage.name,
    ...
    try {
      ...
      if (!existingPin) {
        const { message_id: temporaryMessageAfterCheckId } =
          await this.telegramGeneralService.sendMessage(
            this.bot,
            chatId,
            'Sorry, but I could not find the message you are looking for',
            await this.pinBuddyUtilsService.getKeyboardOptions(chatId),
          );
        await this.pinBuddyUtilsService.sleep(3000);
        await this.telegramGeneralService.deleteMessage(
      ...
      const { message_id: temporaryMessageBeforeSaveId } =
        await this.telegramGeneralService.sendMessage(
          this.bot,
          chatId,
          'OK, I will unpin the message from your saved messages, just a sec...',
          await this.pinBuddyUtilsService.getKeyboardOptions(chatId),
        );
      ...
      const { message_id: temporaryMessageAfterSaveId } =
        await this.telegramGeneralService.sendMessage(
          this.bot,
          chatId,
          `OK, I unpinned the message for you - ${text}`,
          await this.pinBuddyUtilsService.getKeyboardOptions(chatId),
        );
  ...
  async handleClickOnStarredMessage({
    chatId,
    messageId,
  }: Partial<ITelegramMessageData>): Promise<void> {
    try {
      ...
      await this.bot.sendMessage(
        chatId,
        `Click [here](${link}) to jump to the original message.`,
        {
          parse_mode: 'Markdown',
        },
      );
