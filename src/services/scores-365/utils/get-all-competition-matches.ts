 ...
 * using the /games/fixtures endpoint which returns the full schedule.
 * Unlike getCompetitionMatches which uses /games/current (limited window) + gameTime filter.
 */
export async function getAllCompetitionMatches(competitionId: number): Promise<CompetitionDetails> {
  const queryParams = {
