...
export async function getCompetitionTable(competitionId: number): Promise<CompetitionTableDetails> {
  ...
  const competitionTable = relevantCompetitionTableData.rows.map(({ competitor, position, points }) => ({ competitor: { id: competitor.id, name: competitor.name }, position, points }));
  const competitionRawDetails = COMPETITIONS.find((c) => c.id === competitionId);
  const competition = { ..._pick(relevantCompetitionData, ['id', 'name', 'nameForURL']), icon: competitionRawDetails.icon };
