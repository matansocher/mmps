...
export class TelegramGeneralService {
  ...
  getInlineKeyboardMarkup(
    ...
    inlineKeyboard.inline_keyboard = _chunk(
  ...
  async downloadAudioFromVideoOrAudio(
    bot: TelegramBot,
    { video, audio },
    ...
    try {
      ...
      if (video && video.file_id) {
        const videoFileLocalPath = await this.downloadFile(
          bot,
          video.file_id,
          localFilePath,
        );
        audioFileLocalPath =
          await this.utilsService.extractAudioFromVideo(videoFileLocalPath);
        this.utilsService.deleteFile(videoFileLocalPath);
      } else if (audio && audio.file_id) {
        audioFileLocalPath = await this.downloadFile(
          bot,
          audio.file_id,
          localFilePath,
        );
      ...
      this.logger.error(
        this.downloadAudioFromVideoOrAudio.name,
        `err: ${this.utilsService.getErrorMessage(err)}`,
      );
      throw err;
    }
  ...
  sendChatAction(
    bot: TelegramBot,
    chatId: number,
    action = BOT_BROADCAST_ACTIONS.TYPING,
  ): void {
    try {
