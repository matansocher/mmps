...
export class VoicePalService {
  ...
  async handleActionSelection(
    ...
    this.mongoAnalyticLogService.sendAnalyticLog(
      `${analyticAction} - ${ANALYTIC_EVENT_STATES.SET_ACTION}`,
      { chatId },
    );

    this.logger.info(
      this.handleActionSelection.name,
      `chatId: ${chatId}, selection: ${selection}`,
    );
    ...
    await this.telegramGeneralService.sendMessage(
      ...
      chatId,
      replyText,
      this.voicePalUtilsService.getKeyboardOptions(),
    );
  }

  async handleAction(message: Message, userAction: IVoicePalOption) {
    const { chatId, text, audio, video, photo, file } =
      this.telegramGeneralService.getMessageData(message);

    if (!userAction) {
      return this.telegramGeneralService.sendMessage(
        this.bot,
        chatId,
    ...
    const inputErrorMessage =
      this.voicePalUtilsService.validateActionWithMessage(userAction, {
        text,
        audio,
        video,
        photo,
    ...
    try {
      ...
      this.mongoAnalyticLogService.sendAnalyticLog(
        `${analyticAction} - ${ANALYTIC_EVENT_STATES.FULFILLED}`,
        { chatId },
      );
    } catch (err) {
      ...
      this.mongoAnalyticLogService.sendAnalyticLog(
        `${analyticAction} - ${ANALYTIC_EVENT_STATES.ERROR}`,
  ...
  async handleTranscribeAction({
    ...
    try {
      ...
      this.logger.error(
        this.handleTranscribeAction.name,
  ...
  async handleTranslateAction({
    ...
    try {
      ...
      if (text) {
        resText =
          await this.googleTranslateService.getTranslationToEnglish(text);
      } else {
        audioFileLocalPath =
          await this.telegramGeneralService.downloadAudioFromVideoOrAudio(
            this.bot,
      ...
      this.logger.error(
        this.handleTranslateAction.name,
        `error: ${this.utilsService.getErrorMessage(err)}`,
      );
      throw err;
    }
  ...
  async handleTextToSpeechAction({
    ...
    try {
      ...
      await this.telegramGeneralService.sendVoice(
        this.bot,
        chatId,
        audioFilePath,
        this.voicePalUtilsService.getKeyboardOptions(),
      ...
      this.logger.error(
        this.handleTextToSpeechAction.name,
        `error: ${this.utilsService.getErrorMessage(err)}`,
      );
      throw err;
    }
  ...
  async handleSummarizeTextAction({
    chatId,
    text,
  }: Partial<ITelegramMessageData>): Promise<void> {
    try {
      const textSummary = await this.aiService.getChatCompletion(
        SUMMARY_PROMPT,
        text,
      );
      await this.telegramGeneralService.sendMessage(
      ...
      this.logger.error(
        this.handleSummarizeTextAction.name,
        `error: ${this.utilsService.getErrorMessage(err)}`,
      );
      throw err;
    }
  ...
  async handleSummarizeYoutubeVideoAction({
    chatId,
    text,
  }: Partial<ITelegramMessageData>): Promise<void> {
    try {
      const videoId =
        this.youtubeTranscriptService.getYoutubeVideoIdFromUrl(text);
      if (!videoId) {
        await this.telegramGeneralService.sendMessage(
          this.bot,
          chatId,
          NOT_FOUND_VIDEO_MESSAGES.YOUTUBE,
          this.voicePalUtilsService.getKeyboardOptions(),
        );
      }
      const { transcription, errorMessage } =
        await this.youtubeTranscriptService.getYoutubeVideoTranscription(
          videoId,
        );
      if (errorMessage) {
        await this.telegramGeneralService.sendMessage(
      ...
      const summaryTranscription = await this.aiService.getChatCompletion(
        SUMMARY_PROMPT,
        transcription,
      );
      await this.telegramGeneralService.sendMessage(
      ...
      this.logger.error(
        this.handleSummarizeYoutubeVideoAction.name,
        `error: ${this.utilsService.getErrorMessage(err)}`,
      );
      throw err;
    }
  ...
  async handleSummarizeTiktokVideoAction({
    chatId,
    text,
  }: Partial<ITelegramMessageData>): Promise<void> {
    try {
      const audioBuffer =
        await this.socialMediaDownloaderService.getTiktokAudio(text);
      if (!audioBuffer) {
        await this.telegramGeneralService.sendMessage(
          this.bot,
          chatId,
          NOT_FOUND_VIDEO_MESSAGES.TIKTOK,
          this.voicePalUtilsService.getKeyboardOptions(),
        );
      }
      ...
      const transcription =
        await this.aiService.getTranscriptFromAudio(audioFilePath);
      ...
      this.logger.error(
        this.handleSummarizeTiktokVideoAction.name,
        `error: ${this.utilsService.getErrorMessage(err)}`,
      );
      throw err;
    }
  ...
  async handleSummarizeMetaVideoAction({
    chatId,
    text,
  }: Partial<ITelegramMessageData>): Promise<void> {
    try {
      const videoBuffer =
        await this.socialMediaDownloaderService.getMetaVideo(text);
      ...
      const audioFilePath =
        await this.utilsService.extractAudioFromVideo(videoFilePath);
      const transcription =
        await this.aiService.getTranscriptFromAudio(audioFilePath);
      ...
      this.logger.error(
        this.handleSummarizeMetaVideoAction.name,
        `error: ${this.utilsService.getErrorMessage(err)}`,
      );
      throw err;
    }
  ...
  async handleImageGenerationAction({
    chatId,
    text,
  }: Partial<ITelegramMessageData>): Promise<void> {
    try {
      const imageUrl = await this.aiService.createImage(text);
      await this.telegramGeneralService.sendPhoto(
        this.bot,
        chatId,
        imageUrl,
        this.voicePalUtilsService.getKeyboardOptions(),
      ...
      this.logger.error(
        this.handleImageGenerationAction.name,
        `error: ${this.utilsService.getErrorMessage(err)}`,
      );
      throw err;
    }
  ...
  async handleImageAnalyzerAction({
    ...
    try {
      const imageLocalPath = await this.telegramGeneralService.downloadFile(
        this.bot,
      ...
      this.logger.error(
        this.handleImageAnalyzerAction.name,
        `error: ${this.utilsService.getErrorMessage(err)}`,
      );
      throw err;
    }
  ...
  async handleAnalyzerFile({
    chatId,
    file,
  }: Partial<ITelegramMessageData>): Promise<void> {
    try {
      const fileLocalPath = await this.telegramGeneralService.downloadFile(
        this.bot,
        file.file_id,
        LOCAL_FILES_PATH,
      );
      const fileAnalysisText = await this.aiService.analyzeFile(
        FILE_ANALYSIS_PROMPT,
        fileLocalPath,
      );
      await this.telegramGeneralService.sendMessage(
