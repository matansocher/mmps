...
const schema = z.object({
  action: z
    ...
    .describe('Action to perform: "latest_posts" fetches recent tweets, "subscribe" adds hourly new-post notifications for a user, "unsubscribe" removes them, "list" shows current subscriptions'),
...
async function handleSubscribe(username: string): Promise<string> {
  ...
  return `Subscribed to @${username} on Twitter - you will get a notification when they post something new (checked hourly between 11:00-23:00)`;
}
...
export const twitterTool = tool(runner, {
  name: 'twitter',
  description:
    'Fetch the latest posts (tweets) of any public X (Twitter) user by username, or manage hourly new-post notifications: subscribe/unsubscribe to a user, or list current subscriptions. Returns post text, date, link, and engagement metrics when available.',
  schema,
});
