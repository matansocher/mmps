...
export async function getRecipes(chatId: number): Promise<Recipe[]> {
  let recipes = await recipesCacheService.getAllRecipes();
  if (!recipes?.length) {
    recipes = (await getAllRecipes(chatId)) || [];
    await recipesCacheService.saveRecipes(recipes);
  }
  return recipes;
}

export async function getRecipe(chatId: number, recipeId: string): Promise<Recipe> {
  let recipe = await recipesCacheService.getARecipe(recipeId);
  if (!recipe) {
