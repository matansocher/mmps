...
import { getCities, getIsraelCountry, simplifyAreaName } from '.';
...
export function generateCityMap(cityName: string): Canvas {
  ...
  const city = cities.find((c) => simplifyAreaName(c.name) === simplifyAreaName(cityName));
  if (!city) {
    return undefined;
  }
  ...
  // Draw whole country
  const country = getIsraelCountry();
  if (!country.geometry) {
    return;
  }
