...
export class WoltLauncherService {
  private readonly logger = new Logger(WoltLauncherService.name);

  constructor(private readonly bot: Bot) {}

  buildKeyboard(): { inline_keyboard: { text: string; web_app: { url: string } }[][] } | null {
    const url = env.WOLT_MINI_APP_URL;
    if (!url) {
      this.logger.warn('WOLT_MINI_APP_URL not configured');
      return null;
    }
