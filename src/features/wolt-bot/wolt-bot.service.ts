...
export class WoltBotService implements OnModuleInit {
  ...
  async showHandler(message: Message) {
    const { chatId, firstName, lastName } =
      this.telegramGeneralService.getMessageData(message);
    ...
    try {
      const subscriptions =
        await this.mongoSubscriptionService.getActiveSubscriptions(chatId);
      if (!subscriptions.length) {
        const replyText = "You don't have any active subscriptions yet";
        return await this.telegramGeneralService.sendMessage(
          this.bot,
          chatId,
          replyText,
          this.woltUtilsService.getKeyboardOptions(),
      ...
      const promisesArr = subscriptions.map(
        (subscription: SubscriptionModel) => {
          const inlineKeyboardButtons = [
            {
              text: 'Remove',
              callback_data: `remove - ${subscription.restaurant}`,
            },
          ];
      ...
      this.mongoAnalyticLogService.sendAnalyticLog(ANALYTIC_EVENT_NAMES.SHOW, {
        chatId,
      });
      this.logger.info(this.showHandler.name, `${logBody} - success`);
    } catch (err) {
      this.logger.error(
        this.showHandler.name,
  ...
  async textHandler(message: Message) {
    ...
    try {
      this.mongoAnalyticLogService.sendAnalyticLog(
        ANALYTIC_EVENT_NAMES.SEARCH,
        { data: restaurant, chatId },
      );

      const isLastUpdatedTooOld =
        new Date().getTime() - this.woltService.getLastUpdated() >
        TOO_OLD_LIST_THRESHOLD_MS;
      ...
      const restaurants =
        await this.woltService.enrichRestaurants(matchedRestaurants);
      const inlineKeyboardButtons = restaurants.map((restaurant) => {
        const isAvailableComment = restaurant.isOnline
          ? 'Open'
          : restaurant.isOpen
            ? 'Busy'
            : 'Closed';
      ...
      const replyText =
        "Choose one of the above restaurants so I can notify you when it's online";
      await this.telegramGeneralService.sendMessage(
        this.bot,
        chatId,
        replyText,
  ...
  async callbackQueryHandler(callbackQuery: CallbackQuery) {
    const {
      ...
      lastName,
      data: restaurant,
    } = this.telegramGeneralService.getCallbackQueryData(callbackQuery);
    ...
    try {
      ...
      const existingSubscription =
        (await this.mongoSubscriptionService.getSubscription(
          chatId,
          restaurantName,
        )) as SubscriptionModel;

      if (restaurant.startsWith('remove - ')) {
        return await this.handleCallbackRemoveSubscription(
          chatId,
          restaurantName,
      ...
      await this.handleCallbackAddSubscription(
        chatId,
        restaurant,
        existingSubscription,
      );
      ...
      this.logger.error(
        this.callbackQueryHandler.name,
        `error - ${this.utilsService.getErrorMessage(err)}`,
      );
      await this.telegramGeneralService.sendMessage(
        this.bot,
  ...
  async handleCallbackAddSubscription(
    chatId: number,
    restaurant: string,
    ...
    if (existingSubscription) {
      ...
      const restaurantDetails =
        this.woltService
          .getRestaurants()
          .find((r: IWoltRestaurant): boolean => r.name === restaurant) || null;
      if (restaurantDetails && restaurantDetails.isOnline) {
        ...
        const restaurantLinkUrl =
          this.woltService.getRestaurantLink(restaurantDetails);
        const inlineKeyboardButtons = [
          { text: restaurantDetails.name, url: restaurantLinkUrl },
        ];
        form = this.telegramGeneralService.getInlineKeyboardMarkup(
          inlineKeyboardButtons,
        );
      } else {
        replyText =
          `No Problem, you will be notified once ${restaurant} is open.\n\n` +
        ...
        await this.mongoSubscriptionService.addSubscription(
          chatId,
          restaurant,
    ...
    this.mongoAnalyticLogService.sendAnalyticLog(
      ANALYTIC_EVENT_NAMES.SUBSCRIBE,
      { data: restaurant, chatId },
    );
    await this.telegramGeneralService.sendMessage(
      this.bot,
  ...
  async handleCallbackRemoveSubscription(
    chatId: number,
    restaurant: string,
    existingSubscription: SubscriptionModel,
  ) {
    ...
    if (existingSubscription) {
      ...
      await this.mongoSubscriptionService.archiveSubscription(
        chatId,
        restaurantToRemove,
      );
      ...
      replyText =
        `It seems you don\'t have a subscription for ${restaurant}.\n\n` +
        `You can search and register for another restaurant if you like`;
    }
    this.mongoAnalyticLogService.sendAnalyticLog(
      ANALYTIC_EVENT_NAMES.UNSUBSCRIBE,
      { data: restaurant, chatId },
    );
    return await this.telegramGeneralService.sendMessage(
      this.bot,
