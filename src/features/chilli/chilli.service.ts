...
import { DEFAULT_TIMEZONE, isProd, MY_USER_ID, TOODIE_USER_ID } from '@core/config/main.config';
import { Logger } from '@core/utils';
import { AiService, createAgentService } from '@features/chatbot/agent';
import { CHAT_COMPLETIONS_MINI_MODEL } from '@services/openai/constants';
import { getPrompt } from './mongo';

type ChilliUser = 'guz' | 'toodie' | 'unknown';

function identifyUser(chatId: number): ChilliUser {
  if (chatId === MY_USER_ID) return 'guz';
  if (chatId === TOODIE_USER_ID) return 'toodie';
  return 'unknown';
}

function buildUserContext(user: ChilliUser): string {
  switch (user) {
    case 'guz':
    ...
    default:
      return 'המשתמש שמדבר איתך לא מוכר לך. תהיי חשדנית קצת אבל בסדר, כמו שאת עם זרים.';
  }
}
...
export class ChilliService {
  ...
  async processMessage(message: string, chatId: number): Promise<string> {
    try {
      const [prompt, user] = await Promise.all([getPrompt(), Promise.resolve(identifyUser(chatId))]);
