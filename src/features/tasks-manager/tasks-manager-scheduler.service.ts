...
import { ACTION_VALUE_SEPARATOR, ANALYTIC_EVENS, BOT_ACTIONS } from './tasks-manager-bot.config';
...
export class TasksManagerSchedulerService {
  ...
  private async notifyUser(task: any): Promise<void> {
    ...
    try {
      const inlineKeyboardButtons = [{ text: 'Mark as completed', callback_data: `${task._id}${ACTION_VALUE_SEPARATOR}${BOT_ACTIONS.TASK_COMPLETED}` }];
