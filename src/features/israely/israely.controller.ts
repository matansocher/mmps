...
import { IsraelyMongoGameLogService, IsraelyMongoSubscriptionService, IsraelyMongoUserService } from '@core/mongo/israely-mongo';
...
import { generateSpecialMessage, generateStatisticsMessage, getCityByName } from './utils';

const customErrorMessage = 'אופס, קרתה לי תקלה, אבל אפשר לנסות שוב מאוחר יותר 🙁';

@Injectable()
export class IsraelyController implements OnModuleInit {
  private readonly logger = new Logger(IsraelyController.name);
  ...
  constructor(
    ...
    private readonly gameLogDB: IsraelyMongoGameLogService,
  ...
  onModuleInit(): void {
    ...
    const { START, MAP, ACTIONS } = BOT_CONFIG.commands;
  ...
  async mapHandler(message: Message): Promise<void> {
    const { chatId, userDetails } = getMessageData(message);
    try {
      await this.israelyService.mapHandler(chatId);
  ...
  private async callbackQueryHandler(callbackQuery: CallbackQuery): Promise<void> {
    ...
    try {
      ...
      if (![BOT_ACTIONS.MAP].includes(action)) {
        return;
      }
