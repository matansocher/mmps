...
export class TeacherBotService implements OnModuleInit {
  ...
  onModuleInit() {
    const handlers: TelegramBotHandler[] = [
      { regex: TEACHER_BOT_OPTIONS.START, handler: this.startHandler },
      { regex: TEACHER_BOT_OPTIONS.STOP, handler: this.stopHandler },
      { regex: TEACHER_BOT_OPTIONS.COURSE, handler: this.courseHandler },
      { regex: TEACHER_BOT_OPTIONS.LESSON, handler: this.lessonHandler },
      { regex: TEACHER_BOT_OPTIONS.LIST, handler: this.listHandler },
      { regex: TEACHER_BOT_OPTIONS.HISTORY, handler: this.historyHandler },
      { regex: TEACHER_BOT_OPTIONS.ADD, handler: this.addHandler },
      { regex: TEACHER_BOT_OPTIONS.REMOVE, handler: this.removeHandler },
    ];
  ...
  private async handleCommand(message: Message, handler: (chatId: number) => Promise<void>) {
    ...
    try {
      ...
      this.logger.log(`${handler.name} - ${logBody} - success`);
    } catch (err) {
      const errorMessage = getErrorMessage(err);
      this.logger.error(`${handler.name} - ${logBody} - error - ${errorMessage}`);
  ...
  private async addHandler(message: Message) {
    await this.handleCommand(message, async (chatId) => {
      const course = message.text.replace(TEACHER_BOT_OPTIONS.ADD, '').trim();
  ...
  private async removeHandler(message: Message) {
    await this.handleCommand(message, async (chatId) => {
      const courseId = message.text.replace(TEACHER_BOT_OPTIONS.REMOVE, '').trim();
