...
import { SUGAR_CONFIG } from '../sugar.config';
import { AiServiceOptions, InvokeOptions, MessageState } from '../types';

const logger = new Logger('SugarAiService');
...
export class AiService {
  ...
  private async truncateThread(threadId?: string): Promise<void> {
    ...
    try {
      ...
      if (messages.length <= SUGAR_CONFIG.maxThreadMessages) {
        return;
      }
      ...
      if (SUGAR_CONFIG.preserveSystemMessages) {
        messagesToKeep.push(...systemMessages);
      }

      const availableSlots = SUGAR_CONFIG.maxThreadMessages - messagesToKeep.length;

      if (availableSlots > 0) {
        const recentMessages = nonSystemMessages.slice(-availableSlots);

        if (SUGAR_CONFIG.preserveFirstMessage && nonSystemMessages.length > 0 && availableSlots > 1) {
      ...
      logger.log(`Thread ${threadId} truncated from ${messages.length} to ${messagesToKeep.length} messages`);
    } catch (err) {
      logger.error(`Error truncating thread ${threadId}: ${err}`);
    }
  }
