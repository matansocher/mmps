...
import { VoicePalService } from '@services/voice-pal';
...
export class VoicePalBotService implements OnModuleInit {
  ...
  createBotEventListeners(): void {
    this.bot.on('message', (message: Message) =>
      this.messagesAggregatorService.handleIncomingMessage(
        message,
  ...
  async handleMessage(message: Message): Promise<void> {
    const { chatId, telegramUserId, firstName, lastName, username, text } =
    ...
    try {
      ...
      const availableActions = Object.keys(VOICE_PAL_OPTIONS).map(
        (option: string) => VOICE_PAL_OPTIONS[option].displayName,
      );
      if (availableActions.includes(text)) {
        await this.voicePalService.handleActionSelection(text, {
          telegramUserId,
          chatId,
          firstName,
          lastName,
        ...
        const userAction =
          this.userSelectedActionsService.getCurrentUserAction(chatId);
